...
public:
  ...
  int get_setting_id(){return uid_token;}
...
public:
  ...
  int addBooleanSetting(const char *name, bool defaultValue);
  int addRangeSetting(const char *name, uint8_t min, uint8_t max, uint8_t defaultValue, const char *unit);
  int addOptionSetting(const char *name, const char **options, uint8_t count, uint8_t defaultIndex);
  int addSubscreenSetting(const char *name, Screen *targetScreen,ActionCallback action = nullptr);
