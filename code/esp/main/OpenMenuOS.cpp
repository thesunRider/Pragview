    ...
    if (pressDuration < SHORT_PRESS_TIME && !selectIsLongDetected) {
      if (item_selected_settings >= 0 && item_selected_settings < totalSettings) {
        ...
        if (settings[currentSettingIndex]->type == Setting::BOOLEAN) {
          ...
          settings[currentSettingIndex]->action();
...
int Setting::generateId() {
  uid_token = random(999999);
...
Setting* SettingsScreen::getelement(int uid){
    for (int i = 0; i < totalSettings; i++) {
    Setting *s = settings[i];
    if (s != nullptr && s->name != nullptr && s->uid_token == uid) {
      Serial.printf("Found %d ",uid);
      Serial.println(s->name);
      return s;  // Exit after modifying the first matching setting
    }
  }
  return nullptr; 
} 
...
int SettingsScreen::addBooleanSetting(const char *name, bool defaultValue) {
  ensureInitialized();
  ...
  for (int i = 0; i < settings.size(); i++) {
    if (settings[i]->type == Setting::BOOLEAN && strcmp(settings[i]->name, name) == 0) {
      ...
      return settings[i]->get_setting_id();
    }
  }
...
*/
  ...
  totalSettings = settings.size();
  return s->get_setting_id();
}

int SettingsScreen::addRangeSetting(const char *name, uint8_t min, uint8_t max, uint8_t defaultValue, const char *unit = nullptr) {
  ensureInitialized();
  ...
  Serial.printf("Created range with %d \n",s->get_setting_id());
  settings.push_back(s);
  totalSettings = settings.size();
  return s->get_setting_id();
}

int SettingsScreen::addOptionSetting(const char *name, const char **options, uint8_t count, uint8_t defaultIndex = 0) {
  ensureInitialized();
...
int SettingsScreen::addSubscreenSetting(const char *name, Screen *targetScreen,ActionCallback action) {
  ...
  switch (s->type) {
    ...
    case Setting::RANGE:
      // Adjust the range value by the specified direction, within the range limits
          if(s->rangeValue + direction > s->range.max){
            s->rangeValue = s->range.min;
            break;
          }
          if(s->rangeValue + direction < s->range.min){
            s->rangeValue = s->range.max;
            break;
          }
...
void SettingsScreen::modify(const char *label, int uid) {
...
void SettingsScreen::modify(const char *label, const char *name) {
  if (name == nullptr) {
    return;  // Invalid input
  }
